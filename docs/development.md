...
1. Add data for your API in `src/apiDefs/data`.
    ...
    1. If your new API also creates a new category, then you will have to define a new Typescript file in `src/apiDefs/data` that exports an array of `IApiDescription` objects and define your API in there. Then, with the export value of that module as the `apis` value, define your category in `categories.ts`.
    1. Guidelines for (some) `IApiDescription` properties
