...
COPY --chown=jenkins package.json package-lock.json ./
COPY --chown=jenkins src/openAPISpec/package.json src/openAPISpec/package-lock.json ./src/openAPISpec/
RUN npm install

WORKDIR /application/src/openAPISpec
RUN npm install
