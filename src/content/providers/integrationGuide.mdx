...
The OAuth access token should be validated by submitting it to the Lighthouse token validation service. For details on how to use that service, you can consult the [OpenAPI specification](https://sandbox-api.va.gov/internal/auth/docs/v0/validation.json) for the service API.
