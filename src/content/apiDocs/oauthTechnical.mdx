...
| `response_type` | **Required** | One or two of `id_token`, and `code`. `token` and by extension the implicit flow is not supported. See [Token Types](#id-token) for further explanation. |
...
```http
HTTP/1.1 302 Found
Location: https://sandbox-api.va.gov/oauth2/authorization?
...
### Unsupported Flows
We do not support the implicit flow as it is considered insecure. In the future, we may support PKCE, but we do not currently.

If you are building an application that can't securely protect a client secret, such as a single page web app or a mobile app, you must include a sever side component in your architecture. This component will securely store your client secret and make API requests on behalf of your users.
