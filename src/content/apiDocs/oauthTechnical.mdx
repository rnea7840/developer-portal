...
| `aud`           | **Required** | Supported audiences: `api://default`. <p>The resource server an application is making API calls to. See [security](#security-considerations)</p>. |
...
The `aud` query parameter is the FHIR resource server an application makes API calls to. Specifying this parameter in the authorization request prevents applications from leaking `access_tokens` to conterfeit resource servers.
