...
The database that powers the development environment is populated with synthetic patient data provided by MITRE Corporation. The data contains sample VA patient health records (both living and deceased) that mimic real patient demographics. The associated clinical resources include data generated from disease models covering up to a dozen of the most common afflictions.

The Community Care Eligibility API also allows a user/application to retrieve an array of different eligibilities from a synthetic dataset.

## Development API access

The VA API Platform uses the [OpenID Connect](https://openid.net/specs/openid-connect-core-1_0.html) standard to allow VA patients to authorize third-party applications to access data on their behalf. Some Health APIs also use the [SMART on FHIR](http://docs.smarthealthit.org/) profile. Both OpenID Connect and SMART on FHIR are built on OAuth 2.
